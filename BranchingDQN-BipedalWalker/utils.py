...
def save(agent, rewards, args):
    path = './runs/{}-{}'.format(args.env, "current")
    try:
        os.makedirs(path)
    except:
        pass
...
class AgentConfig:
    ...
    def __init__(self,
                 ...
                 target_net_update_freq=300,  # how often
                 target_net_update_tau=.3,    # interpolation parameter for soft update of target model
                 memory_size=100000,          # replay buffer size
                 batch_size=128,
                 learning_starts=5000,        # model updates only if memory has at least this number of transitions
                 max_frames=10000000):        # number of frames over the whole training
